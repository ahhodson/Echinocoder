...
the other by such a column permutation. We write $M\sim M'$ in that case.  Nonetheless, the reader is warned that we will frequently abuse terminology slightly (e.g.~by referring to 
``the multiset M'') in order to avoid having to use tortuous prose like 
 ``the multiset whose contents can be represetad by the matrix $M$''.  

\begin{example}\label{ex:samemultiset}
With $n=5$, $k=2$,
\[
  M=\begin{bmatrix}4&1&4&2&2\\0&2&5&9&9\end{bmatrix}
  \quad\text{and}\quad
  M'=\begin{bmatrix}1&4&4&2&2\\2&5&0&9&9\end{bmatrix}
\]
represent the same multiset $\bigl\{\,(4,0),(1,2),(4,5),(2,9),(2,9)\,\bigr\}$ as $M'$ is $M$
with the first three columns cyclically shifted.  We may therefore write $M'\sim  M$.  Note that two columns of the matrix $M$ happen to be identical, both being $(2,9)$.  Multisets (unlike sets) can contain such repeated elements, although it is also fine if they do not have repeats.
\end{example}

\subsection{What we want to build}

We seek a map $f$ that sends a multiset $M$ (equivalently, its $\sim$-class) to a
point $f(M)\in\R^N$ for some fixed $N$. We demand the map has the following three properties (\textbf{P1}, \textbf{P2} and \textbf{P3})
and refer to any map having them as an \emph{embeddding}.
...
\item \textbf{Injectivity.} $f(M)=f(M')$ \emph{only} when $M\sim M'$. (Distinct
      multisets give distinct outputs --- no information is lost.
...
one is insufficient}. That is the non-trivial content.
...
line, etc.). Their \emph{simplex}, $\Delta$,  is the convex region with those points at its corners:
\[
...
sub-simplex, $C_\sigma$. As $\sigma$ ranges over all $n!$ orderings,
the sub-simplices $C_\sigma$ tile $\Delta$ without overlap except on their measure-zero boundaries (Figure~\ref{fig:sd}).
...
\begin{tikzpicture}[scale=1.5,line join=round]
  ...
  \node[scale=0.8] at (2.27,0.33) {$C_{(2,1,3)}$}; % want this lable to be seem to be in the middled of the e2-f2-g cell.
\end{tikzpicture}
\caption{Barycentric subdivision of a triangle ($n=3$): the three medians cut it
into $3!=6$ sub-simplices, one per ordering of the vertices. The shaded sub-simplex
$C_{(2,1,3)}$ has vertices $\bvec f_2=\bvec e_1$, $\bvec f_2=\tfrac12(\bvec
e_1{+}\bvec e_2)$ and $\bvec f_3=\bvec g=\tfrac13(\bvec e_1{+}\bvec e_2{+}\bvec
e_3)$. The centre $\bvec g$ is shared by all six sub-simplices.}
\label{fig:sd}
\end{figure}
...
point among the sub-simplices is a \emph{continuous} substitute for sorting.

\begin{proposition}[unique sub-simplex, unique coordinates in that sub-simplex]\label{prop:cell}
...
lies in the sub-simplex $C_\sigma$, and the barycentric coordinates of the point in that sub-simplex are, up to the
...
$\sum_r\beta_r=1$. If the sorting order is \emph{strict} the sub-simplex is unique; if
...
shared by the two sub-simplices that differ only by swapping those two ranks.
\end{proposition}
...
\textbf{The point of Proposition~\ref{prop:cell}.} ``Which sub-simplex?'' is decided by
...
discontinuity out of sorting: \emph{the place where the sub-simplex-label is ambiguous
...
prefixes are as follows.  $\vec u_1=\vec e_2$ as the largest value in $x$, the 9,  was in location 2.  $\vec u_2=\vec u_1 + \vec e_4 = \vec e_2+\vec e_4$ as the next largest value in $x$, the 6, was in location 4. The value of $\vec u_3$ is ambiguous. It could be either $\vec u_2+\vec e_1$ or $\vec u_2 + \vec e_3$ since there is a tie for the third largest value in $x$ between two twos -- one in position 1 and one in position 3. Finally $\vec u_4=\vec e_1+\vec e_2+\vec e_3 +\vec e_4$,  unambiguously, as the largest four items are distributed across all four locations.  The linear combination in \eqref{eq:numdecomp} allows $x$ to be reconstructed completely (including order) from the gaps ($\delta_i$) and the prefixes ($\vec u_i$).  The apparent inability to define $\vec u_3$ is not a problem because the tie that leads to the ambiguity also leads to $\delta_3=0$, meaning that $\vec u_3$ is thereby `turned off' in the linear combination in \eqref{eq:numdecomp}. This feature, the fact that the prefix elements change there identity at (and only at) the very points where they become irrelevant, which are the same points at which a sort-order would change, is key to our retaining continuity in the embedding in more complex cases ($k>1$) considered later.
\end{example}
...
skeleton (which sub-simplixes, which pieces) is scale-free, and only the coordinates
carry the scale.
\end{proof}
...
\begin{example}[a column tie, and a dropped cumulative matrix]\label{ex:tie}
Example~\ref{ex:recon} had distinct gaps, so every $\alpha_s>0$ and every $L_s$
was stored. A tie changes that, and it is worth seeing how the algorithm copes.
Take $n=3$, $k=2$,
\[
   M=\begin{bmatrix}5&2&8\\[1pt]4&4&1\end{bmatrix},
   \qquad
   \{\mathbf v_1,\mathbf v_2,\mathbf v_3\}=\{(5,4),(2,4),(8,1)\},
\]
in which $\mathbf v_1$ and $\mathbf v_2$ \emph{tie in row $2$} (both $4$) yet
differ in row $1$. Anchors $m_1=2,\ m_2=1$; balance
$\left[\begin{smallmatrix}3&0&6\\3&3&0\end{smallmatrix}\right]$. The first
subdivision gives
\[
\renewcommand{\arraystretch}{1.2}
\begin{array}{c|l}
\text{row }1:\ v_2{>}v_1{>}v_3 & \{v_2\}\,(5),\ \ \{v_2v_1\}\,(2)\\
\text{row }2:\ v_1\approx v_2{>}v_3 & \{v_1\,\text{or}\,v_2\}\,(0),\ \ \{v_1v_2\}\,(3)
\end{array}
\]
The row-$2$ top-$1$ prefix is \emph{ambiguous} --- which of the tied
$v_1,v_2$? --- and carries gap~$0$. Pooling and sorting, $\delta=(5,4,2,0)$, so
\[
   \alpha=(1,\,2,\,2,\,\mathbf 0):
\]
the last weight vanishes, and the cumulative matrix that would have rested on the
ambiguous piece is \emph{never stored}. The richest stored matrix is
\[
   L^\ast=\begin{bmatrix}1&2&0\\[1pt]1&1&0\end{bmatrix}.
\]
Decoding it: row $1$ reads $(1,2,0)$, giving the clean order $v_2{>}v_1{>}v_3$;
row $2$ reads $(1,1,0)$ --- $v_1$ and $v_2$ sit at the \emph{same} threshold
level, with nothing above to separate them, so the output reports
``$v_1,v_2$ are the top two of row $2$, tied'' and leaves their row-$2$ order
\emph{undetermined}. That is exactly right: they are equal there. Yet nothing is
lost --- the columns stay distinct in $L^\ast$ (their row-$1$ entries are $1$ and
$2$), so row~$1$ keeps $v_1,v_2$ apart and the multiset is recovered in full.
\emph{The algorithm declines to record only what it cannot define, and the tie
costs it nothing.} (Had $v_1,v_2$ been equal in \emph{every} row --- a genuinely
repeated vector --- every piece separating them would carry gap~$0$ and drop,
and the canonical form would simply carry two identical columns: the repeat,
recorded faithfully.)
\end{example}
